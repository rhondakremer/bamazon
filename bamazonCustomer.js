...
function makePurchase(requestedItem, requestedQuantity) {
    connection.query(`SELECT * FROM products where id=${requestedItem}`, function(error, res){
        if (error) {
            ...
            for (let i = 0; i < res.length; i++) {
                const amountAvailable = parseInt(res[i].stock_quantity);
                if (amountAvailable < requestedQuantity){
                    console.log("Sorry! Insufficient quantity!");
                    purchasePrompt();
                } else {
                    const newAvailable = amountAvailable - requestedQuantity;
                    const cost = res[i].price * requestedQuantity;
                    connection.query(`UPDATE products SET stock_quantity = ${newAvailable} WHERE id = ${requestedItem}`);
